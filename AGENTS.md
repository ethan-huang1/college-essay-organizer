...
- Treat AGENT_HANDOFF.md as a continuously maintained source of truth, not a
  ...
  **Failed Approaches** (OVERNIGHT_TASK.md rule 9) — in place after each
  ...
  (Codex or Codex) can resume accurately.
...
`scripts/overnight_handoff.sh` runs **Codex first, then Codex**,
...
Codex phase, then it stops (no multi-round loop). A fail-closed
...
Codex, after Codex, and after Codex.
...
the pipeline still continues to Codex, and reports a distinct **degraded
...
| Code | Meaning |
|---|---|
| 0 | Codex and Codex both exited 0; all validations passed |
| 2 | pre-flight repo/handoff check failed (before Codex ran) |
| 3 | Codex ended nonzero **and** left an unsafe state — Codex never ran |
| 4 | post-Codex validation failed even though Codex itself exited 0 |
| 5 | Codex phase itself ended nonzero |
| 6 | post-Codex validation failed |
| 9 | lock already held — nothing ran, existing lock untouched |
| 10 | pre-Codex auth preflight failed — nothing ran |
| 11 | Codex left a valid checkpoint, but Codex was skipped (pre-Codex auth preflight failed — e.g. Codex's usage is unavailable tonight) |
| 12 | Codex ended nonzero but left a safe checkpoint; Codex then finished successfully — degraded but recovered, never reported as plain `0` |

### Zero incremental spend

This pipeline is designed to spend **nothing** beyond usage already included
in your Codex and ChatGPT subscriptions. Before your first real run:

1. **Manual prerequisite (the script cannot check this for you):** in Codex
...
2. The orchestrator's auth preflight (`run_auth_preflight`, called before
   Codex and again before Codex) fails closed: it aborts if
   `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` is set, if Codex isn't
   authenticated via a `Codex.ai` subscription (checked via
   `Codex auth status --json`: `loggedIn`, `authMethod`, `apiProvider`, and
...
3. `CLAUDE_BUDGET_USD` (default 2, overridable via env var) is an
   **estimated included-usage ceiling** — it caps how far one Codex
...
- **Monitor (while running or after):** `tail -f logs/overnight/<run-id>/orchestrator.log`;
  ...
  `codex.log`/`Codex.log` in the same directory, with `codex.exit`/`Codex.exit`
...
- **Recover from a stopped/failed run:** read `AGENT_HANDOFF.md`, the exit
  ...
  actually active (`ps aux | grep -E 'Codex -p|codex exec'`) before
  removing it by hand.
- **Cancel a running pipeline:** `Ctrl-C` the foreground process, or
  ...
  process directly (`pkill -f 'Codex -p'` / `pkill -f 'codex exec'`) since
