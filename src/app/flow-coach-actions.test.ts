...
vi.mock("@/lib/coaches/flow-coach", async () => {
  const actual = await vi.importActual<typeof import("@/lib/coaches/flow-coach")>("@/lib/coaches/flow-coach");
  return { ...actual, getFlowCoachFindings: vi.fn() };
});

import { requestFlowCoachAction } from "./flow-coach-actions";
import { getFlowCoachFindings } from "@/lib/coaches/flow-coach";
...
describe("requestFlowCoachAction", () => {
  beforeEach(() => {
    vi.mocked(getActiveWorkspaceSnapshot).mockResolvedValue(snapshot() as never);
    vi.mocked(getFlowCoachFindings).mockReset();
    vi.mocked(getFlowCoachFindings).mockResolvedValue(okResult);
  });

  it("returns invalid-input when the essay is not in the active workspace, without calling Travila", async () => {
    const result = await requestFlowCoachAction("missing-essay", ESSAY);

    expect(result).toMatchObject({ status: "error", reason: "invalid-input" });
    expect(getFlowCoachFindings).not.toHaveBeenCalled();
  });
  ...
  it("delegates the sent text with the server-resolved user id", async () => {
    ...
    await requestFlowCoachAction("essay-1", ESSAY);

    expect(getFlowCoachFindings).toHaveBeenCalledWith({ content: ESSAY, userId: "user-1" });
  });

  it("passes the coach result straight through", async () => {
    expect(await requestFlowCoachAction("essay-1", ESSAY)).toBe(okResult);
  });
});
