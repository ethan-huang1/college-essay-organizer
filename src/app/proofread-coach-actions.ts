...
import { type ProofreadCoachResult, getProofreadCoachFindings } from "@/lib/coaches/proofread-coach";
import { getActiveWorkspaceSnapshot } from "@/lib/workspace-session";

export type ProofreadCoachActionResult = ProofreadCoachResult;

/**
 * Ask Travila's Proofread Coach profile for its read on the current draft. No DB
 ...
 */
export async function requestProofreadCoachAction(essayId: string, content: string): Promise<ProofreadCoachActionResult> {
  ...
  return getProofreadCoachFindings({ content, userId: snapshot.user.id });
}
